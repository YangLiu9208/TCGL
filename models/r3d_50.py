...
class ResNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 shortcut_type='B',
                 widen_factor=1.0,
                 with_classifier=False, return_conv=False, n_classes=101):
        super().__init__()
...
if __name__ == '__main__':
    #r3d = R3DNet((1,1,1,1))
    model =  generate_model(model_depth=50, with_classifier=False, return_conv=False)
    inputs = torch.randn(8, 3, 16, 112, 112)
    output = model(inputs)
    print(output.shape)

'''if __name__ == "__main__":
    model = generate_model(model_depth=50)
    checkpoint = torch.load("pretrained_models/r3d50_KM_200ep.pth", map_location='cuda:0')
    model.load_state_dict(checkpoint['state_dict'])
    #feature_dim = model.fc.in_features
    #model.fc = nn.Linear(feature_dim, 101)
    print(model)'''
