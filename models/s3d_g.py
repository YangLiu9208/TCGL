...
class S3D_G(nn.Module):
    def __init__(self, num_classes=101, drop_prob=0.5, in_channel=3, gate=True, with_classifier=False, return_conv=False):
        ...
        if self.return_conv:
            self.feature_pool = nn.MaxPool3d(kernel_size=(2, 7, 7), stride=(2, 7, 7))   # 9216
    ...
    def forward(self, x):
        # ipdb.set_trace()
        x = self.feature(x)               # (batch_size,num_class,7,1,1)
        if self.return_conv:
        ...
        if self.with_classifier:
            ...
            x = x.squeeze(3)                # (batch_size,num_class,7,1)
            x = x.squeeze(3)                # (batch_size,num_class,7)
            x = x.mean(2)                   # (batch_size,num_class)
        return x
...
if __name__ == "__main__":
    model = S3D_G(with_classifier=False, return_conv=False)
    x = torch.rand(8, 3, 4, 112, 112)
    out = model(x)
    print(out.shape)
