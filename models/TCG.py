...
def noise_feature(x, mean=0., std=1.):
    x + torch.randn(x.size(),device=x.device) * std + mean
    return x
...
class SE_Fusion(nn.Module):
    ...
    def forward(self, fea1, fea2, fea3):
        ...
        #return fea1*y_1.expand_as(fea1), fea2*y_2.expand_as(fea2), fea3*y_3.expand_as(fea3) 

class SE_Fusion_Two(nn.Module):
    def __init__(self,channel_1=128,channel_2=256,channel_3=256,reduction=6):
        super(SE_Fusion_Two, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc_1 = nn.Sequential(
            nn.Linear((channel_1+channel_2), (channel_1+channel_2) // reduction, bias=True),
            nn.ReLU(inplace=True),
            nn.Linear( (channel_1+channel_2) // reduction, channel_1, bias=True),
            nn.Sigmoid()
        )
        self.fc_2 = nn.Sequential(
            nn.Linear((channel_1+channel_2), (channel_1+channel_2) // reduction, bias=True),
            nn.ReLU(inplace=True),
            nn.Linear( (channel_1+channel_2) //reduction, channel_2, bias=True),
            nn.Sigmoid()
        )
    ...
    def forward(self, fea1, fea2):
        ...
        z=torch.cat((y_1,y_2),0)

        y_1 =self.fc_1(z).view(c_1)
        y_2 = self.fc_2(z).view(c_2)  
        
        return torch.mul((F.relu(y_1)).expand_as(fea1),fea1), torch.mul((F.relu(y_2)).expand_as(fea2),fea2)
        #return fea1*y_1.expand_as(fea1), fea2*y_2.expand_as(fea2), fea3*y_3.expand_as(fea3) 
...
class SE_Fusion_Four_batch(nn.Module):
    def __init__(self,channel_1=128,channel_2=256,channel_3=256,channel_4=256,reduction=8):
        super(SE_Fusion_Four_batch, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc_1 = nn.Sequential(
            nn.Linear((channel_1+channel_2+channel_3+channel_4), (channel_1+channel_2+channel_3+channel_4) // reduction, bias=True),
            nn.ReLU(inplace=True),
...
class TCG_triple(nn.Module):
    ...
    def __init__(self, base_network, feature_size, tuple_len):
        ...
        """
        super(TCG_triple, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        for i in range(self.tuple_len):
            cl.append(self.base_network(self.frequency(tuple[:, i, :, :, :, :])))
            #cl_shuffle.append(self.base_network(self.adjacent_shuffle_clip(tuple[:, i, :, :, :, :])))
            for j in range(4):
                sub_f.append(self.base_network(self.repeating2(self.frequency(tuple[:, i, :, 4*j:4*(j+1), :, :]))))
...
class TCG_triple_R3D_R21D(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        for i in range(self.tuple_len):
            ...
            #cl_shuffle.append(self.base_network(self.adjacent_shuffle_clip(tuple[:, i, :, :, :, :])))
            for j in range(4):
                sub_f.append(self.base_network(self.frequency(tuple[:, i, :, 4*j:4*(j+1), :, :])))
                #sub_f.append(self.base_network(tuple[:, i, :, 8*j:8*(j+1), :, :]))
                #sub_f.append(self.base_network(self.repeating(self.adjacent_shuffle(tuple[:, i, :, 4*j:4*(j+1), :, :]))))
...
class TCG_triple_R3D_R21D_EightFrame(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_EightFrame, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        for i in range(self.tuple_len):
            ...
            #cl_shuffle.append(self.base_network(self.adjacent_shuffle_clip(tuple[:, i, :, :, :, :])))
            for j in range(8):
                sub_f.append(self.base_network(tuple[:, i, :, 2*j:2*(j+1), :, :]))
                #sub_f.append(self.base_network(tuple[:, i, :, 8*j:8*(j+1), :, :]))
                #sub_f.append(self.base_network(self.repeating(self.adjacent_shuffle(tuple[:, i, :, 4*j:4*(j+1), :, :]))))
...
class TCG_triple_R3D_R21D_noadaptive(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_noadaptive, self).__init__()
...
class TCG_triple_R3D_R21D_twoGCN(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_twoGCN, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        for j in range(cl.size(0)):
            ...
            gf1.append(self.gcn1(cl[j,:,:], edge_index_clip[j]))
            gf_gcn2.append(self.gcn2(gf1[j], edge_index_clip[j]))
            gf1_shuffle.append(self.gcn1(drop_feature(cl[j,:,:], 0.1), edge_index_clip_drop[j]))
            gf2_shuffle.append(self.gcn2(gf1_shuffle[j], edge_index_clip_drop[j])) 


            fea1,fea2,fea3=self.fusion(gf_gcn2[j][0],gf_gcn2[j][1],gf_gcn2[j][2])
            h.append(torch.cat((fea1,fea2,fea3), dim=0))
            gf2.append(self.fc7(h[j]))
            gf2_relu.append(self.relu(gf2[j])) 
            gf2_drop.append(self.dropout(gf2_relu[j]))
...
class TCG_triple_S3D(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_S3D, self).__init__()

        self.base_network = base_network
        self.feature_size = feature_size
        self.tuple_len = tuple_len
        self.class_num = math.factorial(tuple_len)
        self.gcn_f = GCNConv(self.feature_size, 1024)
        self.gcn_f1 = GCNConv(self.feature_size, 1024)
        self.gcn_f2 = GCNConv(self.feature_size, 1024)
        self.gcn_f3 = GCNConv(self.feature_size, 1024)
        self.gcn1 = GCNConv(self.feature_size, 1024)
        self.gcn2 = GCNConv(1024, 1024)

        self.fc7 = nn.Linear(1024*tuple_len, 1024)
        self.fc8 = nn.Linear(1024, self.class_num)
        #self.fc7 = nn.Linear(self.feature_size*2, 512)
        #pair_num = int(tuple_len*(tuple_len-1)/2)
        #self.fc8 = nn.Linear(512*pair_num, self.class_num)
        #self.fc8 = nn.Linear(512, self.class_num)
        ...
        self.fusion=SE_Fusion(1024,1024,1024,6)
...
class TCG_double_R3D_R21D(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_double_R3D_R21D, self).__init__()
        ...
        self.fusion=SE_Fusion_Two(512,512,4)
    ...
    def forward(self, tuple, tuple_orders):
        ...
        edge_index_clip=[]
        edge_index_clip_drop=[]
        edge_index_clip_1=torch.tensor([[0,1],[1,0]],dtype=torch.long).cuda()
        for i in range(tuple_orders.size(0)):
            if torch.equal(tuple_orders[i].cpu(),torch.tensor([0,1])):
                edge_index_clip.append(edge_index_clip_1)
                edge_index_clip_drop.append(dropout_adj(edge_index_clip_1, p=0.2)[0])
            if torch.equal(tuple_orders[i].cpu(),torch.tensor([1,0])):
                edge_index_clip.append(edge_index_clip_1)
                edge_index_clip_drop.append(dropout_adj(edge_index_clip_1, p=0.2)[0])
        #edge_index_clip=torch.tensor(np.kron(np.eye(2), edge_index_clip.numpy())).cuda()
        #edge_index_frames=torch.tensor([[0,0,0,1,2,3,1,1,1,0,2,3,2,2,2,0,1,3,3,3,3,0,1,2],\
        #    [1,2,3,0,0,0,0,2,3,1,1,1,0,1,3,2,2,2,0,1,2,3,3,3]],dtype=torch.long).cuda()
        edge_index_frames=torch.tensor([[0,1,1,2,2,3],[1,0,2,1,3,2]],dtype=torch.long).cuda()
        sub_f = []  # clip sub-frames features
        sub_shuffle_f = []
        cl= []  # clip features
        cl_shuffle= []  # clip features
        ...
        for j in range(cl.size(0)):
            ...
            fea1,fea2=self.fusion(gf1[j][0],gf1[j][1])
            h.append(torch.cat((fea1,fea2), dim=0))
            gf2.append(self.fc7(h[j]))
            gf2_relu.append(self.relu(gf2[j])) 
            gf2_drop.append(self.dropout(gf2_relu[j]))
            gf3.append(self.fc8(gf2_drop[j]))
        ...
        return contrast_loss_1, contrast_loss_2,contrast_loss_clip, gf3      

class TCG_triple_R3D_R21D_TwoFrame(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_TwoFrame, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        #edge_index_frames=torch.tensor([[0,0,0,1,2,3,1,1,1,0,2,3,2,2,2,0,1,3,3,3,3,0,1,2],\
        #    [1,2,3,0,0,0,0,2,3,1,1,1,0,1,3,2,2,2,0,1,2,3,3,3]],dtype=torch.long).cuda()
        edge_index_frames=torch.tensor([[0,1],\
            [1,0]],dtype=torch.long).cuda()
        sub_f = []  # clip sub-frames features
        sub_shuffle_f = []
        cl= []  # clip features
        cl_shuffle= []  # clip features
...
class TCG_triple_R3D_R21D_OneFrame(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_OneFrame, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        #edge_index_frames=torch.tensor([[0,0,0,1,2,3,1,1,1,0,2,3,2,2,2,0,1,3,3,3,3,0,1,2],\
        #    [1,2,3,0,0,0,0,2,3,1,1,1,0,1,3,2,2,2,0,1,2,3,3,3]],dtype=torch.long).cuda()
        edge_index_frames=torch.tensor([[0],\
            [0]],dtype=torch.long).cuda()
        sub_f = []  # clip sub-frames features
        sub_shuffle_f = []
        cl= []  # clip features
        cl_shuffle= []  # clip features
        for i in range(self.tuple_len):
            ...
            #cl_shuffle.append(self.base_network(self.adjacent_shuffle_clip(tuple[:, i, :, :, :, :])))
            for j in range(1):
                sub_f.append(self.base_network(tuple[:, i, :, 16*j:16*(j+1), :, :]))
                #sub_f.append(self.base_network(tuple[:, i, :, 8*j:8*(j+1), :, :]))
                #sub_f.append(self.base_network(self.repeating(self.adjacent_shuffle(tuple[:, i, :, 4*j:4*(j+1), :, :]))))
...
class TCG_triple_R3D_R21D_noise(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_noise, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        contrast_loss_clip=0.0
        for j in range(cl.size(0)):
            sub_f1_shuffle_drop.append(noise_feature(sub_f1[j,:,:], mean=0., std=1.))
            sub_f2_shuffle_drop.append(noise_feature(sub_f2[j,:,:], mean=0., std=1.))
            sub_f3_shuffle_drop.append(noise_feature(sub_f3[j,:,:], mean=0., std=1.))
            sub_f1_gcn.append(self.gcn_f1(sub_f1[j], edge_index_frames)) 
            sub_f2_gcn.append(self.gcn_f2(sub_f2[j], edge_index_frames)) 
            sub_f3_gcn.append(self.gcn_f3(sub_f3[j], edge_index_frames)) 
            sub_f1_gcn_shuffle.append(self.gcn_f1(sub_f1_shuffle_drop[j], edge_index_frames)) 
            sub_f2_gcn_shuffle.append(self.gcn_f2(sub_f2_shuffle_drop[j], edge_index_frames)) 
            sub_f3_gcn_shuffle.append(self.gcn_f3(sub_f3_shuffle_drop[j], edge_index_frames)) 

            gf1.append(self.gcn1(cl[j,:,:], edge_index_clip[j]))
            gf1_shuffle.append(self.gcn1(noise_feature(cl[j,:,:], mean=0., std=1.), edge_index_clip[j]))
...
class TCG_triple_R3D_R21D_edge(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_edge, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        for j in range(cl.size(0)):
            ...
            sub_f2_gcn.append(self.gcn_f2(sub_f2[j], edge_index_frames)) 
            sub_f3_gcn.append(self.gcn_f3(sub_f3[j], edge_index_frames)) 
            sub_f1_gcn_shuffle.append(self.gcn_f1(sub_f1[j], edge_index_frames_shuffle_drop1)) 
            sub_f2_gcn_shuffle.append(self.gcn_f2(sub_f2[j], edge_index_frames_shuffle_drop2)) 
            sub_f3_gcn_shuffle.append(self.gcn_f3(sub_f3[j], edge_index_frames_shuffle_drop3)) 

            gf1.append(self.gcn1(cl[j,:,:], edge_index_clip[j]))
            gf1_shuffle.append(self.gcn1(cl[j,:,:], edge_index_clip_drop[j]))
...
class TCG_triple_R3D_R21D_node(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_node, self).__init__()
    ...
    def forward(self, tuple, tuple_orders):
        ...
        for j in range(cl.size(0)):
            ...
            gf1.append(self.gcn1(cl[j,:,:], edge_index_clip[j]))
            gf1_shuffle.append(self.gcn1(drop_feature(cl[j,:,:], 0.1), edge_index_clip[j]))
...
class TCG_triple_R3D_R21D_FCA(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_FCA, self).__init__()
...
class TCG_triple_R3D_R21D_FCA2(nn.Module):
    """Video clip order prediction with PFE (Pairwire Feature Extraction), the same as OPN."""
    def __init__(self, base_network, feature_size, tuple_len):
        """
        Args:
            feature_size (int): 512
        """
        super(TCG_triple_R3D_R21D_FCA2, self).__init__()
