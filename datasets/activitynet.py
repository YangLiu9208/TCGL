...
class ActivityDataset(Dataset):
    ...
    """
    def __init__(self, root_dir, clip_len, split='1', train=True, transforms_=None, test_sample_num=50):
        ...
        if self.train:
            train_split_path = os.path.join(root_dir, 'split', 'trainlist.txt')
            self.train_split = pd.read_csv(train_split_path, header=None, sep=' ')[0]
            self.train_label = pd.read_csv(train_split_path, header=None, sep=' ')[1]
        else:
            test_split_path = os.path.join(root_dir, 'split', 'vallist.txt')
            self.test_split = pd.read_csv(test_split_path, header=None, sep=' ')[0]
            self.test_label = pd.read_csv(test_split_path, header=None, sep=' ')[1]
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            videolabel = self.train_label[idx]
        else:
            videoname = self.test_split[idx]
            videolabel = self.test_label[idx]
        class_idx = int(videolabel)-1
        filename = os.path.join(self.root_dir, 'ActivityNet-V1.3-resized', videoname)
        #print(filename)
...
class ActivityDataset_linear(Dataset):
    """UCF101 dataset for recognition. The class index start from 0.
...
class UCF101Dataset_Gram(Dataset):
    """UCF101 dataset for recognition. The class index start from 0.
...
class UCF101VCOPDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        filename = os.path.join(self.root_dir, 'video', videoname)#.replace('\\', '/')
        #videodata = skvideo.io.vread(filename)
        videodata = VideoReader(filename,ctx=cpu(0)) 
        #length, height, width, channel = videodata.shape
        length = len(videodata)
...
class UCF101VCOPDataset_double(Dataset):
    """UCF101 dataset for video clip order prediction. Generate clips and permutes them on-the-fly.
    Need the corresponding configuration file exists. 
    ...
    def __init__(self, root_dir, clip_len, interval, tuple_len, train=True, transforms_=None):
        ...
        self.transforms_ = transforms_
        self.toPIL = transforms.ToPILImage()
        self.tuple_total_frames = 2*clip_len * tuple_len + interval * (tuple_len - 1)

        if self.train:
            vcop_train_split_name = 'vcop_train_{}_{}_{}_double.txt'.format(clip_len, interval, tuple_len)
            vcop_train_split_path = os.path.join(root_dir, 'split', vcop_train_split_name)
            self.train_split = pd.read_csv(vcop_train_split_path, header=None)[0]
        else:
            vcop_test_split_name = 'vcop_test_{}_{}_{}_double.txt'.format(clip_len, interval, tuple_len)
            vcop_test_split_path = os.path.join(root_dir, 'split', vcop_test_split_name)
            self.test_split = pd.read_csv(vcop_test_split_path, header=None)[0]
    ...
    def __getitem__(self, idx):
        ...
        clip_start = tuple_start
        for _ in range(self.tuple_len):
            clip = videodata.get_batch(list(range(clip_start,clip_start + 2*self.clip_len,2)))
            tuple_clip.append(clip)
            clip_start = clip_start + 2*self.clip_len + self.interval
...
class UCF101VCOPDataset_triple(Dataset):
    """UCF101 dataset for video clip order prediction. Generate clips and permutes them on-the-fly.
    Need the corresponding configuration file exists. 
    ...
    def __init__(self, root_dir, clip_len, interval, tuple_len, train=True, transforms_=None):
        ...
        self.transforms_ = transforms_
        self.toPIL = transforms.ToPILImage()
        self.tuple_total_frames = 3*clip_len * tuple_len + interval * (tuple_len - 1)

        if self.train:
            vcop_train_split_name = 'vcop_train_{}_{}_{}_triple.txt'.format(clip_len, interval, tuple_len)
            vcop_train_split_path = os.path.join(root_dir, 'split', vcop_train_split_name)
            self.train_split = pd.read_csv(vcop_train_split_path, header=None)[0]
        else:
            vcop_test_split_name = 'vcop_test_{}_{}_{}_triple.txt'.format(clip_len, interval, tuple_len)
            vcop_test_split_path = os.path.join(root_dir, 'split', vcop_test_split_name)
            self.test_split = pd.read_csv(vcop_test_split_path, header=None)[0]
...
class UCF101VFCOPDataset(Dataset):
    """UCF101 dataset for video clip-frame order prediction. Generate clips and permutes them on-the-fly.
    Need the corresponding configuration file exists. 
    ...
    def __getitem__(self, idx):
        ...
        tuple_clip_random = []
        clip_frame=[]
        clip_frame_order = list(range(0, self.clip_len))
        ...
        for _ in range(self.tuple_len):
            clip = videodata[clip_start: clip_start + self.clip_len]
            random.shuffle(clip_frame_order)
            clip_random=clip.asnumpy()[clip_frame_order]
            tuple_clip.append(clip)
            tuple_clip_random.append(clip_random)
            clip_start = clip_start + self.clip_len + self.interval

        clip_and_order = list(zip(tuple_clip, tuple_order))
        clip_and_order_random = list(zip(tuple_clip_random, tuple_order))
        # random shuffle for train, the same shuffle for test
        if self.train:
            random.shuffle(clip_and_order)
            random.shuffle(clip_and_order_random)
        else:
            random.seed(idx)
            random.shuffle(clip_and_order)
        tuple_clip, tuple_order = zip(*clip_and_order)
        tuple_clip_random, tuple_order_random = zip(*clip_and_order_random)

        if self.transforms_:
            trans_tuple = []
            trans_tuple_random = []
            for clip, clip_random in zip(tuple_clip, tuple_clip_random):
                trans_clip = []
                trans_clip_random = []
                # fix seed, apply the sample `random transformation` for all frames in the clip 
                seed = random.random()
                for frame, frame_random in zip(clip.asnumpy(), clip_random):
                    random.seed(seed)
                    frame = self.toPIL(frame) # PIL image
                    frame_random = self.toPIL(frame_random) # PIL image
                    frame = self.transforms_(frame) # tensor [C x H x W]
                    frame_random = self.transforms_(frame_random) # tensor [C x H x W]
                    trans_clip.append(frame)
                    trans_clip_random.append(frame_random)
                # (T x C X H x W) to (C X T x H x W)
                trans_clip = torch.stack(trans_clip).permute([1, 0, 2, 3])
                trans_clip_random = torch.stack(trans_clip_random).permute([1, 0, 2, 3])
                trans_tuple.append(trans_clip)
                trans_tuple_random.append(trans_clip_random)
            tuple_clip = trans_tuple
            tuple_clip_random = trans_tuple_random
        else:
            tuple_clip = [torch.tensor(clip) for clip in tuple_clip]
            tuple_clip_random = [torch.tensor(clip_random) for clip_random in tuple_clip_random]

        return torch.stack(tuple_clip), torch.tensor(tuple_order), torch.stack(tuple_clip_random), torch.tensor(tuple_order), idx
...
def gen_ucf101_vcop_splits(root_dir, clip_len, interval, tuple_len):
    ...
    def _video_longer_enough(filename):
        ...
        path = os.path.join(root_dir, 'video', filename).replace('\\', '/')
        metadata = ffprobe(path)['video']
        return eval(metadata['@nb_frames']) >= min_video_len

    train_split = pd.read_csv(os.path.join(root_dir, 'split', 'trainlist01.txt').replace('\\', '/'), header=None, sep=' ')[0]
    train_split = train_split[train_split.apply(_video_longer_enough)]
    train_split.to_csv(vcop_train_split_path, index=None)

    test_split = pd.read_csv(os.path.join(root_dir, 'split', 'testlist01.txt').replace('\\', '/'), header=None, sep=' ')[0]
...
if __name__ == '__main__':
    ...
    gen_ucf101_vcop_splits('data/ucf101', 64, 8, 3)
...
class UCF101VCOPDataset_color(Dataset):
    """UCF101 dataset for video clip order prediction. Generate clips and permutes them on-the-fly.
    Need the corresponding configuration file exists. 
    ...
    Args:
        ...
        transforms_ (object): composed transforms which takes in PIL image and output tensors.
    """
    def __init__(self, root_dir, clip_len, interval, tuple_len, train=True, transforms_=None, color_jitter_=None):
        self.root_dir = root_dir
        self.clip_len = clip_len
        self.interval = interval
        self.tuple_len = tuple_len
        ...
        if self.transforms_:
            ...
            for clip in tuple_clip:
                ...
                for frame in clip.asnumpy():
                    ...
                    frame = self.color_jitter_(frame)
                     # tensor [C x H x W]
                    trans_clip.append(frame)
                # (T x C X H x W) to (C X T x H x W)
                trans_clip = torch.stack(trans_clip).permute([1, 0, 2, 3])
