...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='r3d50', help='c3d/r3d/r21d/i3d/r3d50/s3d')
...
if __name__ == '__main__':
    ...
    if args.model == 'c3d':
        ...
        #base = resnet50(sample_size=224,sample_duration=16,with_classifier=False, return_conv=False).to(device)
        #base = r3d50(with_classifier=False, return_conv=False).to(device)
        base =generate_model(model_depth=50, with_classifier=False, return_conv=False).to(device)

    tcg = TCG_triple_R3D_R21D_FCA2(base_network=base, feature_size=2048, tuple_len=args.tl).cuda()
    ...
    if args.mode == 'train':  ########### Train #############
        if args.ckpt:  # resume training
            ...
            if args.desp:
                exp_name = 'UCF101_R3D50_TCG224_{}_cl{}_it{}_tl{}_{}_{}'.format(args.model, args.cl, args.it, args.tl, args.desp, time.strftime('%m%d%H%M'))
            else:
                exp_name = 'UCF101_R3D50_TCG224_{}_cl{}_it{}_tl{}_{}'.format(args.model, args.cl, args.it, args.tl, time.strftime('%m%d%H%M'))
